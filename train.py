...
def get_parser():
    ...
    parser.add_argument("--attention_setting", type=str, choices=["v0", "v1", "v2"], required=True,
                        help="Setting for decoder cross-attention module, benefits for distinguish language.\
                              v0:don't distinguish language (defalut setting of offically released XLM); \
                              v1:specify the final linear layer of cross-attention by language (defalut setting of offically released MASS); \
                              v2:specify the entire cross-attention by language")
...
def main(params):
    ...
    for _ in range(trainer.epoch, params.max_epoch):
        ...
        while trainer.n_sentences < trainer.epoch_size:
            ...
            if params.stbt and trainer.epoch >= params.stbt_start_epoch:
                # online self-training + back-translation steps
                for lang1, lang2, lang3 in shuf_order(params.bt_steps):
                    trainer.st_bt_step(lang1, lang2, lang3, params.lambda_bt, params.lambda_st)
            else:        
