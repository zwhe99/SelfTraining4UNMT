...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            # Ratio of data used
            if splt == 'train' and params.mono_data_ratio < 1:
                n_batches = data['mono_stream'][lang][splt].n_batches 
                a = 0
                b = int(n_batches * params.data_ratio)
                data['mono_stream'][lang][splt].select_data(a, b)

            # for denoising auto-encoding / online back-translation / mass pretraining, we need a non-stream (batched) dataset
...
def load_para_data(params, data):
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if splt == 'train' and (src, tgt) not in required_para_train:
                continue
...
def check_data_params(params):
    ...
    # dont check the traning set when eval_only
    if params.eval_only:
        params.para_dataset = {
            (src, tgt): {
                ...
                if splt != 'train' or (src, tgt) in required_para_train
            } for src in params.langs for tgt in params.langs
            if (src, tgt) in required_para 
        }
    else:
        params.para_dataset = {
            (src, tgt): {
