...
class Evaluator(object):
    ...
    def get_iterator(self, data_set, lang1, lang2=None, stream=False):
        ...
        if lang2 is None:
            ...
            iterator = self.data['para'][(lang1, lang2)][data_set].get_iterator(
                shuffle=False,
                group_by_size=True,
                n_sentences=n_sentences
            )
    ...
    def create_src_ref_files(self):
        """
        Create source and reference files for BLEU evaluation.
        """
        params = self.params
        params.ref_paths = {}
        ...
        for (lang1, lang2), v in self.data['para'].items():
            ...
            for data_set in ['valid', 'test']:
                ...
                # define data paths
                lang1_path = os.path.join(params.hyp_path, 'src.{0}-{1}.{2}.{3}.txt'.format(lang1, lang2, lang1, data_set))
                lang2_path = os.path.join(params.hyp_path, 'ref.{0}-{1}.{2}.{3}.txt'.format(lang1, lang2, lang2, data_set))

                # store data paths
                # params.ref_paths[(lang1, lang2, lang1, data_set)] = lang1_path
                params.ref_paths[(lang1, lang2, lang2, data_set)] = lang2_path
        ...
        with torch.no_grad():
            ...
            for data_set in ['valid', 'test']:
                ...
                if len(params.mass_steps) > 0:
                    scores['%s_mass_ppl' % data_set] = np.mean([scores['%s_%s-%s_mass_ppl' % (data_set, lang, lang)] for lang in params.mass_steps])
                    scores['%s_mass_acc' % data_set] = np.mean([scores['%s_%s-%s_mass_acc' % (data_set, lang, lang)] for lang in params.mass_steps])
...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        if eval_bleu:
            ...
            hyp_name = 'hyp{0}.{1}-{2}.{3}.{4}.txt'.format(scores['epoch'], lang1, lang2, lang2, data_set)
            hyp_path = os.path.join(params.hyp_path, hyp_name)
            ref_path = params.ref_paths[(lang1, lang2, lang2, data_set)]
