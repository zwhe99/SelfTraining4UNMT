...
from src.data.dictionary import BT_TAG, ST_TAG
...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        if self.epoch_size == -1:
            self.epoch_size = max([len(self.data['para'][(src, tgt)]["train"]) for (src, tgt) in self.data['para'].keys()])
        ...
        if params.multi_gpu and params.amp == -1:
            ...
            for name in self.MODEL_NAMES:
                # ref: https://github.com/facebookresearch/XLM/issues/211
        ...
        self.stats = OrderedDict(
            ...
            [('CL-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.bt_steps] +
            [('ACL-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.bt_steps] +
            [('ST-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.bt_steps] +
            [('AST-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.bt_steps]
        )
    ...
    def get_iterator(self, iter_name, lang1, lang2, stream):
        ...
        if lang2 is None:
            ...
            iterator = self.data['para'][(lang1, lang2)]['train'].get_iterator(
                shuffle=True,
                group_by_size=self.params.group_by_size,
                n_sentences=-1,
            )
    ...
    def reload_checkpoint(self):
        ...
        for name in self.MODEL_NAMES:
            ...
            # check the parameter of encoder
            # Refs:
            # https://github.com/facebookresearch/XLM/issues/109#issuecomment-504916842
            # https://github.com/facebookresearch/XLM/issues/51#issuecomment-480832990
            if name == 'encoder':
                strict = False
                ckpt_encoder_keys = [k if not k.startswith('module.') else k[len('module.'):] for k in data['encoder'].keys()]
                model_encoder_keys = [k if not k.startswith('module.') else k[len('module.'):] for k, _ in getattr(self, 'encoder').named_parameters()]
                for k in ckpt_encoder_keys:
                    if k not in model_encoder_keys:
                        logger.warning(f"Unexpected key of encoder: {k}. Check https://github.com/facebookresearch/XLM/issues/109#issue")
                for k in model_encoder_keys:
                    if k not in ckpt_encoder_keys:
                        logger.warning(f"Missing key of encoder: {k}.")
            else:
                strict = True

            ckpt_multigpu = True if  all([k.startswith('module.') for name in self.MODEL_NAMES for k in data[name].keys()]) else False
            if ckpt_multigpu == self.params.multi_gpu:
                getattr(self, name).load_state_dict(data[name], strict=strict)
            elif ckpt_multigpu == True and self.params.multi_gpu == False:
                getattr(self, name).load_state_dict({k[len('module.'):]: v for k, v in data[name].items()}, strict=strict)
            else:
                getattr(self, name).load_state_dict({'module.' + k: v for k, v in data[name].items()}, strict=strict) 
...
class EncDecTrainer(Trainer):
    ...
    def st_bt_step(self, lang1, lang2, lang3, lambda_bt, lambda_st):
        """Self-training Back-translation step for machine translation.

        Args:
            lang1 (str): 
            lang2 (str): 
            lang3 (str): 
            lambda_bt (float): BT loss coefficient
            lambda_st (float): ST loss coefficient
        """
        assert lambda_bt >= 0 and lambda_st >= 0
        assert lang1 == lang3 and lang1 != lang2 and lang2 is not None

        if lambda_bt == 0 and lambda_st == 0:
            return
        params = self.params
        _encoder = self.encoder.module if params.multi_gpu else self.encoder
        _decoder = self.decoder.module if params.multi_gpu else self.decoder
        ...
        # generate source batch
        # x and len contains current batch of sentences and sentences lengths respectively, including eos and bos.
        x1, len1 = self.get_batch('bt', lang1) # x1:[max_len1, bs], len1:[bs]. 
        langs1 = x1.clone().fill_(lang1_id) # create matrices filled with lang_id for language embedding
        ...
        with torch.no_grad():
            ...
            # encode source sentence 
            # return a representaion of all tokens in x1, including bos and eos
            enc1 = _encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False) 
            enc1 = enc1.transpose(0, 1) # enc1: [bs, max_len1, dim]. 

            # given enc1 and len1 , translate it to language2 specified by lang2_id
            # x2 and len2 contains generated sentences and sentences lengths respectively, including eos and bos.
            # x2:[max_len2, bs], len2:[bs], x2_scores:[max_len2-1, bs, num_words] "max_len2-1" means <BOS> is not included.
            x2, len2, x2_scores = _decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5), return_scores=True) 
            langs2 = x2.clone().fill_(lang2_id) 
        ...
        # encode generate sentence
        # return a representaion of all tokens in x2, including bos and eos
        enc2 = self.encoder('fwd', x=x2, lengths=len2, langs=langs2, causal=False)
        enc2 = enc2.transpose(0, 1) # enc2: [bs, max_len2, dim]


        # decode to original sentence
        # given enc2 as input, decode it to lang1 using teacher-forcing mode
        # dec3: [max_len1, bs, dim]
        dec3 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=len2) 

        # words to predict
        # y1 is a concatenated groud-truth, contains eos of each sentences but without bos.
        alen1 = torch.arange(len1.max(), dtype=torch.long, device=len1.device)
        pred_mask1 = alen1[:, None] < len1[None] - 1  # do not predict anything given the last target word. pred_mask1: [max_len1, bs]
        y1 = x1[1:].masked_select(pred_mask1[:-1]).detach() 

        # back translation loss
        _, bt_loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask1, y=y1, get_scores=False)

        self.stats[('BT-%s-%s-%s' % (lang1, lang2, lang3))].append(bt_loss.item())
        

        # ----self-training-----
        # add noise
        (_x1, _len1) = self.add_noise(x1, len1) if params.stbt_add_noise else (x1, len1)

        # encode the original sentence
        # return a representaion of all tokens in x1, including bos and eos
        enc1 = self.encoder('fwd', x=_x1, lengths=_len1, langs=langs1, causal=False) 
        enc1 = enc1.transpose(0, 1) # enc1: [bs, max_len1, dim]. 

        # decode to back-translated sentence
        # given enc1 as input, decode it to lang2 using teacher-forcing mode
        # dec4: [max_len2, bs, dim]
        dec4 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1) 

        # words to predict
        # y2 is a concatenated groud-truth(synthesised), contains eos of each sentences but without bos.
        alen2 = torch.arange(len2.max(), dtype=torch.long, device=len2.device)
        pred_mask2 = alen2[:, None] < len2[None] - 1  # do not predict anything given the last target word. pred_mask2: [max_len2, bs]
        y2 = x2[1:].masked_select(pred_mask2[:-1]).detach()
        __, st_loss = self.decoder('predict', tensor=dec4, pred_mask=pred_mask2, y=y2, get_scores=False)

        if st_loss is not None and lambda_st != 0:
            self.stats[('ST-%s-%s-%s' % (lang1, lang2, lang3))].append(st_loss.item())
            final_loss = lambda_bt * bt_loss  + lambda_st * st_loss
        else:
            self.stats[('ST-%s-%s-%s' % (lang1, lang2, lang3))].append(0)
            final_loss = lambda_bt * bt_loss
